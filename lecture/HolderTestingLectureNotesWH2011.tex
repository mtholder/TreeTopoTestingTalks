...
\subsection{Notation}
$X$ will denote the data. $x_i$ denotes column $i$ in the matrix.\\
$T_i$ denotes tree i. $\hat{T}$ is the MLE of the tree. \\
$^{(j)}$ superscripts in parentheses denote bootstrap replicates. $X^{(j)}$ would be the $j$-th bootstrap replicate dataset; $\hat{T}^{(j)} would be the MLE of the tree for that dataset.\\
...
Below is a depiction of the $\delta$ test statistic and the expected value of the test statistic if the null were true. the expectation is 0.\par
