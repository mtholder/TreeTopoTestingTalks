...
Many test rely on the $\delta$ test statistic, because (according to the \href{http://en.wikipedia.org/wiki/Neyman-Pearson_lemma}{Neyman-Pearson Lemma}) the a test using the likelihood ratio will result on the most powerful test.
Having high statistical power means that you are able to reject the null hypothesis when it is false.
...
\begin{wrapfigure}{r}{0.5\textwidth}
  \begin{center}
   \vspace{-40pt}
    ...
    \put(-240,10){\myFigLabel{figUncenteredSH3Counter}: SH Test uncentered $\lnL$ for {\color{blue} $T_1$},  {\color{green} $T_2$}, {\color{red} $T_3$}}
...
\begin{wrapfigure}{r}{0.5\textwidth}
  \begin{center}
   \vspace{-40pt}
    ...
    \put(-200,10){\myFigLabel{figSHTestStat3Counter}: SH Test draws from the null }
   \vspace{-0pt}
   \vspace{-20pt}
  \end{center}
\end{wrapfigure}
...
Bootstrap replicate is a case in which $T_2$ had the highest $\lnL$, but does not have the highest value after centering.
...
\subsubsection{Effect of using a larger candidate set of trees}
In order to correct for the selection bias introduced by using the ML tree to calculate the $\delta$ test statistic, the SH Test must be given the set of trees that were considered during the search.
Using all possible trees is a valid way to perform the test, but this leads to computational hurdles and a loss of statistical power.
...
\subsection{Performing the SH test in PAUP}
\begin{verbatim}
GetTrees file = candidate.tre mode = 3 ;
LScore / SHTest=RELL ;
\end{verbatim}

\subsection{Summary of the SH Test}
The SH Test is a version of the KH Test that is appropriate to use when you have more than two candidate trees that you would like to test.
